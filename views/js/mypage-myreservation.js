export const generateReservationCards = async (myReservations) => {
  ...
  cardList.innerHTML = myReservations
    .map((reservation) => {
      let addReviewDisplay = "inline-block";
      let fixReservationDisplay = "none";
      let statusStyle = "green";
      if (reservation.status === "예약중") {
        addReviewDisplay = "none";
        fixReservationDisplay = "inline-block";
        statusStyle = "black";
      }
      console.log(addReviewDisplay, fixReservationDisplay);
      return `
        <div class="container">
          ...
          <div href="/html/pet-sitter-by-id.html?petSitterId=${
            ...
            <div class="fs-3">
              <P class="h4">Career<strong class="h3"><strong> ${
                reservation.petSitterCareer
          ...
          <div href="/" class="m-4 align-self-center me-auto text-decoration-none">
            <div class="fs-3 text-center">
              ...
              <P id="cancel" class="h4 text-dark"><strong style="color: ${statusStyle};">${
        reservation.status
      }</strong></P>
              <button type="button" id="fixReservation:petSitterId:${reservation.petSitterId}:reservationId:${
        reservation.id
      }" class="btn btn-outline-dark" style="display: ${fixReservationDisplay};" data-bs-toggle="modal" data-bs-target="#fixReservationModal${
        reservation.id
      }">예약수정</button>
              <button type="button" style="display: ${fixReservationDisplay};" id="deleteReservation:petSitterId:${
        reservation.petSitterId
      }:reservationId:${reservation.id}" class="btn btn-outline-danger">예약취소</button>
      <button type="button" style="display: ${addReviewDisplay};" id="createReviewOpenModal:${
        reservation.petSitterId
      }" class="btn btn-outline-dark" data-bs-toggle="modal" data-bs-target="#createReviewModal">리뷰작성</button>
            </div>
          </div>
        </div>
